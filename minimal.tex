% My general-use, fairly minimal \LaTeX document, from which I normally start
...
\usepackage{microtype}					% refine typograpy
...
\usepackage[backend=biber, natbib=true, backref=true, url=false]{biblatex}	% modern bibliography
	\addbibresource{/afs/psi.ch/user/h/haberthuer/Documents/library.bib}	% from this file kept in Mendeley
\usepackage{graphicx}					% we generally have figures
\usepackage{subfig}					% and subfigres
\usepackage[list-units=single,detect-all]{siunitx}	% use SI units
...
\usepackage{blindtext}					% we don't use 'blindtext' in general, but it makes this document nice
...
% Helpers
\newcommand{\imsize}{\linewidth}i		 	% general figure size
...
\recalctypearea	% recalculate type area if we did something funky with fonts
